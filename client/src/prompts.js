// prompts.js — the text we send to Gemini for each AI feature.
// Each prompt ends with a strict "return ONLY JSON" instruction so the reply
// can be JSON.parse'd directly (see gemini.js).
import { WEEK, MEMBERS } from './constants.js';

// "Bhargav & Rupa" — derived from MEMBERS so renaming the family is one edit.
const FAMILY = MEMBERS.join(' & ');

// Only send the fields the model needs — smaller prompts are cheaper and
// keep the model from echoing irrelevant data back.
...
export const schedulePrompt = (tasks) =>
  `Schedule tasks for ${FAMILY} (couple with toddler), week ${WEEK[0]}–${WEEK[6]}.
...
export const reprioritizePrompt = (tasks) =>
  `Reprioritize these overdue tasks for ${FAMILY}. Assign new priority (1=Critical–5=Someday) and reschedule this week.
...
export const chatPrompt = (tasks, user, message) =>
  `Manage tasks for ${FAMILY} (toddler family). Current tasks: ${JSON.stringify(tasks.map(t => ({
...
{"action":"add","task":{"title":"...","category":"tasks|grocery|toddler|dinner|date|workout","priority":1-5,"assignee":"${MEMBERS.join('|')}|Both","dueDate":"YYYY-MM-DD or null","duration":30,"notes":"","recurrence":"daily|weekly|null","done":false,"scheduledDate":null,"scheduledTime":null,"completedDates":[]}}
